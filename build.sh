...
# 关键点 2e：重放 patches/0005 —— 消息详情超长正文分块渐进渲染（幂等）。
#           纯前端渲染逻辑 + 文案，无依赖变更。
if git -C "$REPO" apply --check "$ROOT/patches/0005-perf-message-detail-progressive-render.patch" 2>/dev/null; then
  git -C "$REPO" apply "$ROOT/patches/0005-perf-message-detail-progressive-render.patch"
  log "已重放 patches/0005（消息详情超长正文渐进渲染）"
elif grep -q "splitMarkdownChunks" "$REPO/frontend/apps/web-ele/src/views/message/detail.vue" 2>/dev/null; then
  log "patches/0005 已在源码中，跳过重放"
else
...
# 关键点 2g：重放 patches/0007 —— 长文详情分段按需渲染（加载更多+滚动续填）（幂等）。
#           前端 detail.vue 切分器升级 + loadMore 控制条 + locale 键，无依赖变更。
if git -C "$REPO" apply --check "$ROOT/patches/0007-perf-segmented-loadmore-rendering.patch" 2>/dev/null; then
  git -C "$REPO" apply "$ROOT/patches/0007-perf-segmented-loadmore-rendering.patch"
  log "已重放 patches/0007（长文分段按需渲染）"
elif grep -q "loadMoreSegments" "$REPO/frontend/apps/web-ele/src/views/message/detail.vue" 2>/dev/null; then
  log "patches/0007 已在源码中，跳过重放"
else
  echo "::error::patches/0007 既无法应用也不在源码中（上游基线可能已变更）"; exit 1
fi
...
# docker load 兼容性更好
# --load 对所有驱动均合法：docker 驱动=默认行为；docker-container 驱动(CI)必须显式
# 加载才能 docker save，否则产物留在构建缓存报 "No such image"（曾致 CI 失败）
docker buildx build --platform linux/arm/v7 --provenance=false --sbom=false --load \
